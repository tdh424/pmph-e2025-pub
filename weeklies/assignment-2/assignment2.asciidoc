...
command `./test_pbb 100000 1024` should manifest it.
(Or set 1024 as the second argument of `test_pbb` in `Makefile`.)
...
*When compiling/running with block size `1024` remember* to set the value of `ELEMS_PER_THREAD` to `6` (in `Makefile`, and if not set there in file `constants.cuh`) otherwise you will also get other errors!
